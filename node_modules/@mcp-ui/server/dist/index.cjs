"use strict";Object.defineProperty(exports,Symbol.toStringTag,{value:"Module"});const l="ui/resourceUri",c="mcpui.dev/ui-",u=`var __defProp = Object.defineProperty;
...
`;function o(e){const t=e?JSON.stringify(e):"{}";return`
<script>
(function() {
  'use strict';
...
<\/script>
`.trim()}const m=`var __defProp = Object.defineProperty;
var __defNormalProp = (obj, key, value) => key in obj ? __defProp(obj, key, { enumerable: true, configurable: true, writable: true, value }) : obj[key] = value;
var __publicField = (obj, key, value) => {
  __defNormalProp(obj, typeof key !== "symbol" ? key + "" : key, value);
  return value;
...
`;function d(e){const t=e?{timeout:e.timeout}:{},n=JSON.stringify(t);return`
<script>
(function() {
  'use strict';
...
<\/script>
`.trim()}function r(e){const t={...e.resourceProps??{}};if(e.uiMetadata||e.metadata){const n=Object.fromEntries(Object.entries(e.uiMetadata??{}).map(([i,s])=>[`${c}${i}`,s]));t._meta={...n,...e.metadata??{},...t._meta??{}}}return t}function h(e){if(typeof Buffer<"u")return Buffer.from(e,"utf-8").toString("base64");if(typeof TextEncoder<"u"&&typeof btoa<"u"){const n=new TextEncoder().encode(e);let i="";const s=8192;for(let a=0;a<n.length;a+=s)i+=String.fromCharCode(...n.slice(a,a+s));return btoa(i)}else{console.warn("MCP-UI SDK: Buffer API and TextEncoder/btoa not available. Base64 encoding might not be UTF-8 safe.");try{return btoa(e)}catch{throw new Error("MCP-UI SDK: Suitable UTF-8 to Base64 encoding method not found, and fallback btoa failed.")}}}function p(e){if(e){if(e.appsSdk?.enabled)return e.appsSdk.mimeType??"text/html+skybridge";if(e.mcpApps?.enabled)return"text/html+mcp"}}function g(e,t){if(!t)return e;const n=[];if(t.appsSdk?.enabled){const a=o(t.appsSdk.config);n.push(a)}if(t.mcpApps?.enabled){const a=d(t.mcpApps.config);n.push(a)}if(n.length===0)return e;const i=n.join(`
`);let s;return e.includes("<head>")?s=e.replace("<head>",`<head>
${i}`):e.includes("<html>")?s=e.replace("<html>",`<html>
<head>
${i}
</head>`):s=`${i}
${e}`,s}function f(e){let t,n;if(e.content.type==="rawHtml"){if(!e.uri.startsWith("ui://"))throw new Error("MCP-UI SDK: URI must start with 'ui://' when content.type is 'rawHtml'.");if(t=e.content.htmlString,typeof t!="string")throw new Error("MCP-UI SDK: content.htmlString must be provided as a string when content.type is 'rawHtml'.");e.adapters?(t=g(t,e.adapters),n=p(e.adapters)??"text/html"):n="text/html"}else if(e.content.type==="externalUrl"){if(!e.uri.startsWith("ui://"))throw new Error("MCP-UI SDK: URI must start with 'ui://' when content.type is 'externalUrl'.");const s=e.content.iframeUrl;if(typeof s!="string")throw new Error("MCP-UI SDK: content.iframeUrl must be provided as a string when content.type is 'externalUrl'.");t=s,n="text/uri-list"}else if(e.content.type==="remoteDom"){if(!e.uri.startsWith("ui://"))throw new Error("MCP-UI SDK: URI must start with 'ui://' when content.type is 'remoteDom'.");if(t=e.content.script,typeof t!="string")throw new Error("MCP-UI SDK: content.script must be provided as a string when content.type is 'remoteDom'.");n=`application/vnd.mcp-ui.remote-dom+javascript; framework=${e.content.framework}`}else{const s=e.content;throw new Error(`MCP-UI SDK: Invalid content.type specified: ${s}`)}let i;switch(e.encoding){case"text":i={uri:e.uri,mimeType:n,text:t,...r(e)};break;case"blob":i={uri:e.uri,mimeType:n,blob:h(t),...r(e)};break;default:{const s=e.encoding;throw new Error(`MCP-UI SDK: Invalid encoding type: ${s}`)}}return{type:"resource",resource:i,...e.embeddedResourceProps??{}}}function I(e){window.parent&&window.parent.postMessage(e,"*")}const w={UI_MESSAGE_RECEIVED:"ui-message-received",UI_MESSAGE_RESPONSE:"ui-message-response",UI_SIZE_CHANGE:"ui-size-change",UI_LIFECYCLE_IFRAME_READY:"ui-lifecycle-iframe-ready",UI_LIFECYCLE_IFRAME_RENDER_DATA:"ui-lifecycle-iframe-render-data",UI_RAWHTML_CONTENT:"ui-html-content"},M={WAIT_FOR_RENDER_DATA:"waitForRenderData"};function y(e,t){return{type:"tool",payload:{toolName:e,params:t}}}function A(e){return{type:"prompt",payload:{prompt:e}}}function R(e){return{type:"link",payload:{url:e}}}function P(e,t){return{type:"intent",payload:{intent:e,params:t}}}function S(e){return{type:"notify",payload:{message:e}}}exports.InternalMessageType=w;exports.RESOURCE_URI_META_KEY=l;exports.ReservedUrlParams=M;exports.createUIResource=f;exports.getAdapterMimeType=p;exports.getAppsSdkAdapterScript=o;exports.getMcpAppsAdapterScript=d;exports.postUIActionResult=I;exports.uiActionResultIntent=P;exports.uiActionResultLink=R;exports.uiActionResultNotification=S;exports.uiActionResultPrompt=A;exports.uiActionResultToolCall=y;exports.wrapHtmlWithAdapters=g;
